...
- dec4480: Replaced the native better-sqlite3 driver with Node's built-in node:sqlite, so running membank through npx no longer fails with ERR_DLOPEN_FAILED when the invoking Node version differs from the one that populated the npx cache. All four packages now declare a Node >=24 engine, giving a clear EBADENGINE error on older runtimes instead of a cryptic module failure.
- dec4480: Upgraded the build toolchain to TypeScript 7, so the published type declarations are now generated by the new compiler.
- Updated dependencies [dec4480]
- Updated dependencies [dec4480]
- Updated dependencies [dec4480]
  - @membank/core@0.23.0
